...
function getCodexThinkingEffortOptions(t: (key: string) => string) {
  return [
    { id: "", label: t("configForm.thinkingEffort.auto") },
    { id: "minimal", label: t("configForm.thinkingEffort.minimal") },
    { id: "low", label: t("configForm.thinkingEffort.low") },
    { id: "medium", label: t("configForm.thinkingEffort.medium") },
    { id: "high", label: t("configForm.thinkingEffort.high") },
    { id: "xhigh", label: t("configForm.thinkingEffort.xHigh") },
  ] as const;
}

function getOpenCodeThinkingEffortOptions(t: (key: string) => string) {
  return [
    { id: "", label: t("configForm.thinkingEffort.auto") },
    { id: "minimal", label: t("configForm.thinkingEffort.minimal") },
    { id: "low", label: t("configForm.thinkingEffort.low") },
    ...
    { id: "high", label: t("configForm.thinkingEffort.high") },
    { id: "xhigh", label: t("configForm.thinkingEffort.xHigh") },
    { id: "max", label: t("configForm.thinkingEffort.max") },
  ] as const;
}

function getCursorModeOptions(t: (key: string) => string) {
  return [
    { id: "", label: t("configForm.thinkingEffort.auto") },
    { id: "plan", label: t("configForm.thinkingEffort.plan") },
    { id: "ask", label: t("configForm.thinkingEffort.ask") },
  ] as const;
}

function getClaudeThinkingEffortOptions(t: (key: string) => string) {
  return [
    { id: "", label: t("configForm.thinkingEffort.auto") },
    { id: "low", label: t("configForm.thinkingEffort.low") },
    { id: "medium", label: t("configForm.thinkingEffort.medium") },
  ...
  return (
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {isDirty && !props.hideInlineSave && (
        <div className="sticky top-0 z-10 flex items-center justify-end px-4 py-2 bg-background/90 backdrop-blur-sm border-b border-primary/20">
          <div className="flex items-center gap-3">
            ...
            >
              {!isCreate && props.isSaving ? t("detail.saving") : t("detail.save")}
            </Button>
          </div>
        </div>
      )}
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("detail.defaultAgentName")} hint={help.name}>
              <DraftInput
            ...
            <Field label={t("detail.titlePlaceholder")} hint={help.title}>
              <DraftInput
            ...
            <Field label={t("detail.reportsTo")} hint={help.reportsTo}>
              <ReportsToPicker
        ...
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            ...
            >
              ...
              >
                <option value="">{t("configForm.companyDefaultLocal")}</option>
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("configForm.permissionsAndConfiguration")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("configForm.permissionsAndConfiguration")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              ...
              {adapterType === "opencode_local"
                ...
                <p className="text-xs text-muted-foreground">
                  {t("configForm.liveOpencodeDiscovery", { name: currentDefaultEnvironment.name })}
                </p>
              )}
                  ...
                  {adapterType === "codex_local" &&
                    ...
                    currentThinkingEffort === "minimal" && (
                      <p className="text-xs text-amber-400">
                        {t("configForm.codexSearchWarning")}
                      </p>
                    )}
                </>
              )}
              {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
                <>
                  <Field label={t("configForm.bootstrapPrompt.label")} hint={help.bootstrapPrompt}>
                    <MarkdownEditor
                      value={eff(
                        "adapterConfig",
                      ...
                      placeholder={t("configForm.bootstrapPrompt.placeholder")}
                      contentClassName="min-h-[44px] text-sm font-mono"
                      imageUploadHandler={async (file) => {
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
                    {t("configForm.bootstrapPrompt.warning")}
                  </div>
                </>
              )}
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-md border border-border/70 px-3 py-2">
                <ToggleField
                  label={t("configForm.continueAfterMaxTurnStop")}
...
export function AdapterEnvironmentResult({ result }: { result: AdapterEnvironmentTestResult }) {
  const { t } = useTranslation("agents");
  const statusLabel =
    result.status === "pass" ? t("configForm.adapterEnvironmentResult.passed") : result.status === "warn" ? t("configForm.adapterEnvironmentResult.warnings") : t("configForm.adapterEnvironmentResult.failed");
  const statusClass =
    result.status === "pass"
      ...
      <div className="mt-2 space-y-1.5">
        {result.checks.map((check, idx) => (
          <div key={`${check.code}-${idx}`} className="text-[11px] leading-relaxed break-words">
            ...
            {check.hint && <span className="block opacity-90 break-words">{t("configForm.adapterEnvironmentResult.hint")}: {check.hint}</span>}
          </div>
        ))}
      </div>
    </div>
          ...
          <button type="button" className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected
                ? selected.label
                : value
                  || (allowDefault ? (defaultLabel ?? t("configForm.modelDropdown.default")) : required ? t("configForm.modelDropdown.selectModelRequired") : t("configForm.modelDropdown.selectModel"))}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          <div className="relative mb-1">
            <input
              ...
              placeholder={creatable ? t("configForm.modelDropdown.searchModelsCreate") : t("configForm.modelDropdown.searchModels")}
              value={modelSearch}
          ...
          {onDetectModel && !modelSearch.trim() && (
            ...
            >
              ...
              {detectingModel ? t("configForm.modelDropdown.detecting") : detectedModel ? (detectModelLabel?.replace(/^Detect\b/, t("configForm.modelDropdown.redetect")) ?? t("configForm.modelDropdown.redetect")) : (detectModelLabel ?? t("configForm.modelDropdown.detect"))}
            </button>
          )}
          {onRefreshModels && !modelSearch.trim() && (
            ...
            >
              ...
              {refreshingModels ? t("configForm.modelDropdown.refreshing") : t("configForm.modelDropdown.refreshModels")}
            </button>
          )}
          {value && (!models.some((m) => m.id === value) || promotedModelIds.has(value)) && (
            ...
            >
              ...
              <span className="shrink-0 ml-auto text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-green-500/15 text-green-400 border border-green-500/20">
                {t("configForm.modelDropdown.current")}
              </span>
            </button>
          )}
          {detectedModel && detectedModel !== value && (
            ...
            >
              ...
              <span className="shrink-0 ml-auto text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-blue-500/15 text-blue-400 border border-blue-500/20">
                {t("configForm.modelDropdown.detected")}
              </span>
            </button>
          )}
          {detectedModelCandidates
              ...
              return (
                ...
                >
                  ...
                  <span className="shrink-0 ml-auto text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-sky-500/15 text-sky-400 border border-sky-500/20">
                    {t("configForm.modelDropdown.config")}
                  </span>
                </button>
              );
            })}
          <div className="max-h-[240px] overflow-y-auto">
            {allowDefault && (
              ...
              >
                {t("configForm.modelDropdown.default")}
              </button>
            )}
            {canCreateManualModel && (
              ...
              >
                <span>{t("configForm.modelDropdown.useManualModel")}</span>
            ...
            {filteredModels.length === 0 && !canCreateManualModel && promotedModelIds.size === 0 && (
              <div className="px-2 py-2 space-y-2">
                <p className="text-xs text-muted-foreground">
                  {onDetectModel
                    ? (emptyDetectHint ?? t("configForm.modelDropdown.noModelDetected"))
                    : t("configForm.modelDropdown.noModelsFound")}
                </p>
              </div>
            )}
          </div>
...
function CheapModelSection({
  ...
  const placeholderHint = adapterDefaultModel
    ? t("configForm.cheapModel.adapterDefault", { model: adapterDefaultModel })
    : t("configForm.cheapModel.noAdapterDefault");
  return (
    <div className="rounded-md border border-border/70 bg-muted/20 p-3 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("configForm.cheapModel.cheapModel")}</div>
          <p className="text-xs text-muted-foreground">
            {t("configForm.cheapModel.description")}
          </p>
        </div>
      ...
      {enabled && !model && adapterDefaultModel ? (
        <p className="text-[11px] text-muted-foreground">
          {t("configForm.cheapModel.fallbackTo", { model: adapterDefaultModel })}
        </p>
      ) : null}
      {enabled && !model && !adapterDefaultModel ? (
        <p className="text-[11px] text-amber-500">
          {t("configForm.cheapModel.noCheapModelWarning")}
        </p>
      ) : null}
    </div>
  );
...
function ThinkingEffortDropdown({
  ...
  return (
    <Field label={t("configForm.thinkingEffort.thinkingEffort")} hint={help.thinkingEffort}>
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>{selected?.label ?? t("configForm.thinkingEffort.auto")}</span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
