...
export function ProfileSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("settings:profile.instanceSettings") },
  ...
  function resolveProfileName() {
    return name.trim() || sessionQuery.data?.user.name || t("settings:profile.board");
  }
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings:profile.failedToUpdateProfile"));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("settings:profile.selectCompanyBeforeUpload"));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings:profile.failedToUploadAvatar"));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings:profile.failedToRemoveAvatar"));
    },
  });

  if (sessionQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("settings:profile.loadingProfile")}</div>;
  }

  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : t("settings:profile.failedToLoadProfile")}
      </div>
    );
  }

  const currentName = name.trim() || sessionQuery.data.user.name || t("settings:profile.board");
  ...
  const uploadHint = selectedCompany
    ? t("settings:profile.storedInPaperclip", { companyName: selectedCompany.name })
    : t("settings:profile.selectCompanyToUpload");
    ...
    <div className="max-w-4xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("settings:profile.profile")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("settings:profile.controlHowYourAccountAppears")}
        </p>
      </div>
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-[24px] border border-border/70 bg-background/92 p-5 shadow-[0_18px_44px_-28px_rgba(0,0,0,0.45)] backdrop-blur-sm">
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                ...
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("settings:profile.clickAvatarToUpload")} {uploadHint}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div className="space-y-2">
            ...
            <Input
              ...
              maxLength={120}
              placeholder={t("settings:profile.board")}
            />
            <p className="text-xs text-muted-foreground">
              {t("settings:profile.shownInSidebar")}
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">
              {t("settings:profile.emailManagedByAuth")}
            </p>
          </div>
