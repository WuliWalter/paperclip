...
export function buildRoutineGroups(
  ...
  if (groupByValue === "project") {
    ...
    return Object.keys(groups)
      .sort((left, right) => {
        const leftLabel = left === "__no_project" ? t("group.noProjectFallback") : (projectById.get(left)?.name ?? t("group.unknownProject"));
        const rightLabel = right === "__no_project" ? t("group.noProjectFallback") : (projectById.get(right)?.name ?? t("group.unknownProject"));
        return leftLabel.localeCompare(rightLabel);
      })
      .map((key) => ({
        key,
        label: key === "__no_project" ? t("group.noProjectFallback") : (projectById.get(key)?.name ?? t("group.unknownProject")),
        items: groups[key]!,
      }));
  }
  ...
  return Object.keys(groups)
    .sort((left, right) => {
      const leftLabel = left === "__unassigned" ? t("group.unassigned") : (agentById.get(left)?.name ?? t("group.unknownAgent"));
      const rightLabel = right === "__unassigned" ? t("group.unassigned") : (agentById.get(right)?.name ?? t("group.unknownAgent"));
      return leftLabel.localeCompare(rightLabel);
    })
    .map((key) => ({
      key,
      label: key === "__unassigned" ? t("group.unassigned") : (agentById.get(key)?.name ?? t("group.unknownAgent")),
      items: groups[key]!,
    }));
}
...
export function Routines() {
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("toast.failedToUpdateRoutine"),
        body: mutationError instanceof Error ? mutationError.message : t("errors.updateFailed"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("toast.routineRunFailed"),
        body: mutationError instanceof Error ? mutationError.message : t("errors.runFailed"),
        tone: "error",
      });
    },
  });
  ...
  const routineGroups = useMemo(
    () => buildRoutineGroups(sortedRoutines, routineViewState.groupBy, projectById, agentById, (key) => t(key)),
  ...
  const concurrencyPolicyDescriptions = useConcurrencyPolicyDescriptions((key) => t(key));
  const catchUpPolicyDescriptions = useCatchUpPolicyDescriptions((key) => t(key));
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        ...
        <TabsContent value="routines" className="space-y-4">
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm text-muted-foreground">
              {t("routineCount", { count: (routines ?? []).length })}
            </p>
            <div className="flex items-center gap-1">
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-xs" title={t("sort.label")}>
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-xs" title={t("group.label")}>
                    <Layers className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1" />
                    <span className="hidden sm:inline">{t("group.label")}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-44 p-0">
                  <div className="p-2 space-y-0.5">
      ...
      >
        ...
        >
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.2em] text-muted-foreground">{t("newRoutine.breadcrumb")}</p>
              <p className="text-sm text-muted-foreground">
                {t("newRoutine.subtitle")}
              </p>
            </div>
            <Button
              variant="ghost"
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            <div className="px-5 pt-5 pb-3">
              <textarea
                ...
                placeholder={t("newRoutine.titlePlaceholder")}
                rows={1}
                value={draft.title}
                onChange={(event) => {
            ...
            <div className="px-5 pb-3">
              <div className="overflow-x-auto overscroll-x-contain">
                <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
                  <span>{t("newRoutine.for")}</span>
                  <InlineEntitySelector
                    ref={assigneeSelectorRef}
                    value={draft.assigneeAgentId}
                    options={assigneeOptions}
                    recentOptionIds={recentAssigneeIds}
                    placeholder={t("newRoutine.assignee")}
                    noneLabel={t("newRoutine.noAssignee")}
                    searchPlaceholder={t("newRoutine.searchAssignees")}
                    emptyMessage={t("newRoutine.noAssigneesFound")}
                    ...
                    renderTriggerValue={(option) =>
                      option ? (
                        ...
                        <span className="text-muted-foreground">{t("newRoutine.assignee")}</span>
                      )
                    }
                    renderOption={(option) => {
                  ...
                  />
                  <span>{t("newRoutine.in")}</span>
                  <InlineEntitySelector
                    ref={projectSelectorRef}
                    value={draft.projectId}
                    options={projectOptions}
                    recentOptionIds={recentProjectIds}
                    placeholder={t("newRoutine.project")}
                    noneLabel={t("newRoutine.noProject")}
                    searchPlaceholder={t("newRoutine.searchProjects")}
                    emptyMessage={t("newRoutine.noProjectsFound")}
                    onChange={(projectId) => {
                    ...
                    renderTriggerValue={(option) =>
                      option && currentProject ? (
                        ...
                        <span className="text-muted-foreground">{t("newRoutine.project")}</span>
                      )
                    }
                    renderOption={(option) => {
                      if (!option.id) return <span className="truncate">{option.label}</span>;
            ...
            <div className="border-t border-border/60 px-5 py-4">
              <MarkdownEditor
                ...
                placeholder={t("newRoutine.instructionsPlaceholder")}
                bordered={false}
            ...
            <div className="border-t border-border/60 px-5 py-3">
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                  <div>
                    <p className="text-sm font-medium">{t("newRoutine.advancedDeliverySettings")}</p>
                    <p className="text-sm text-muted-foreground">{t("newRoutine.advancedDeliveryHint")}</p>
                  </div>
                ...
                <CollapsibleContent className="pt-3">
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("newRoutine.concurrency")}</p>
                      <Select
                    ...
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("newRoutine.catchUp")}</p>
                      <Select
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-muted-foreground">
              {t("newRoutine.afterCreationHint")}
            </div>
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              >
                <Plus className="mr-2 h-4 w-4" />
                {createRoutine.isPending ? t("newRoutine.creating") : t("newRoutine.create")}
              </Button>
              {createRoutine.isError ? (
                <p className="text-sm text-destructive">
                  {createRoutine.error instanceof Error ? createRoutine.error.message : t("newRoutine.createFailed")}
                </p>
              ) : null}
            </div>
          </div>
