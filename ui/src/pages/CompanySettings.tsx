...
export function CompanySettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("company.breadcrumbCompany"), href: "/dashboard" },
      { label: t("company.breadcrumbSettings") }
    ]);
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3 rounded-md border border-border px-4 py-4">
          <Field label={t("company.companyName")} hint={t("company.companyNameHint")}>
            <input
        ...
        <div className="space-y-3 rounded-md border border-border px-4 py-4">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  {logoUrl && (
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {clearLogoMutation.isPending ? t("company.removing") : t("company.removeLogo")}
                      </Button>
                    </div>
                  )}
                  ...
                  {logoUploadMutation.isPending && (
                    <span className="text-xs text-muted-foreground">{t("company.uploadingLogo")}</span>
                  )}
                </div>
              </Field>
              <Field
              ...
              <Field
                label={t("company.attachmentSizeLimit")}
                hint={`${t("company.acceptedRange")}: 1-${MAX_COMPANY_ATTACHMENT_MAX_MIB} ${t("company.mib")}.`}
              >
                <div className="flex flex-col gap-1.5">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-xs text-muted-foreground">{t("company.mib")}</span>
                  </div>
                  {!attachmentMaxValid && (
                    <span className="text-xs text-destructive">
                      {t("company.enterWholeNumber", { max: MAX_COMPANY_ATTACHMENT_MAX_MIB })}
                    </span>
                  )}
                </div>
              </Field>
      ...
      <div className="space-y-4" data-testid="company-settings-team-section">
        ...
        <div className="rounded-md border border-border px-4 py-3">
          <ToggleField
            label={t("company.requireBoardApproval")}
        ...
        <div className="space-y-3 rounded-md border border-border px-4 py-4">
          <div className="flex items-center gap-1.5">
            <span className="text-xs text-muted-foreground">
              {t("company.generateInviteSnippet")}
            </span>
            <HintIcon text={t("company.inviteHintIcon")} />
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              {inviteMutation.isPending
                ...
                : t("company.generateOpenClawInvite")}
            </Button>
          </div>
          {inviteError && (
      ...
      <div className="space-y-4">
        <div className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
          {t("company.companyPackages")}
        </div>
        <div className="rounded-md border border-border px-4 py-4">
          <p className="text-sm text-muted-foreground">
            {t("company.importExportMoved")}{" "}
            <a href="/org" className="underline hover:text-foreground">{t("company.orgChart")}</a> {t("company.header")}.
          </p>
            ...
            <Button
              ...
              onClick={() => {
                ...
                const confirmed = window.confirm(
                  t("company.archiveConfirm", { name: selectedCompany.name })
                );
                if (!confirmed) return;
                const nextCompanyId =
                  companies.find(
